(() => {
  ...
  const VERSION = '4.12.1-square-moshimo';
  ...
  function affiliateParameters(link, destination) {
    ...
    return {
      ...
      affiliate_key: link.dataset.affiliateKey || container?.dataset.affiliateKey || (destination.hostname === 'px.a8.net' ? destination.searchParams.get('a8mat') || '' : (destination.hostname === 'af.moshimo.com' ? ['a_id','p_id','pc_id','pl_id'].map(key => destination.searchParams.get(key) || '').join(':') : '')),
      content_language: document.documentElement.lang || '',
