...
export async function generateStoreDxPlatform({root, outputRoot, site, productCatalog, articleRecords, renderDirectoryCard, header, footer, currentLabel, esc, writeFile}) {
  ...
  for (const language of site.languages) {
    ...
    const title = isJa ? '店舗DX総合ガイド｜POS・決済・予約・LINE・会計を整理 | Luqevora.com' : 'Store DX Hub: POS, Payments, Booking, LINE, and Accounting | Luqevora.com';
    ...
    const graph = {'@context':'https://schema.org','@graph':[
      ...
      {'@type':'BreadcrumbList',itemListElement:[{'@type':'ListItem',position:1,name:'Luqevora.com',item:`${site.baseUrl}/${language}/`},{'@type':'ListItem',position:2,name:isJa?'店舗DX':'Store DX',item:canonical}]}
    ]};
    ...
    const diagnosisGraph = {'@context':'https://schema.org','@graph':[
      ...
      {'@type':'BreadcrumbList',itemListElement:[{'@type':'ListItem',position:1,name:'Luqevora.com',item:`${site.baseUrl}/${language}/`},{'@type':'ListItem',position:2,name:isJa?'店舗DX':'Store DX',item:canonical},{'@type':'ListItem',position:3,name:isJa?'無料診断':'Free assessment',item:diagnosisCanonical}]}
    ]};
